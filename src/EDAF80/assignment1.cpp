...
#define		BODYID_NEPTUNE	8
	...
	while (!glfwWindowShouldClose(window)) {
		...
		//LamLe: loop until the stack is empty
		while (!CelestialBodyStack.empty())
		{
			//LamLe: get the node on top of stack
			curNode = CelestialBodyStack.top();
			//LamLe: pop it out of stack
			CelestialBodyStack.pop();
			//LamLe: Process current node
			glm::mat4 retMat = 
		...
		// nodes.
		//glm::mat4 retMat = earth.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), glm::mat4(1.0f), show_basis);
		//moon.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), retMat, show_basis);
